...
//				Pixel messagePixel = messagePixels[r][c];
				if(pixelObj.getGreen() == pixelObj.getBlue() - 4){
	...
	public void encode(Picture messagePict) {
		...
		Pixel[][] pixels = this.getPixels2D();		//"this" refers to redMoto
			...
			for(int c=0; c<pixels[0].length; c++) {
				...
				if(messagePixel.getColor().equals(Color.BLACK)){
					//if(pixelObj.getGreen() == pixelObj.getBlue() && pixelObj.getGreen() == pixelObj.getRed()){
						//checking if the green, red, and blue values are EQUAL is too rare...
					if(Math.abs(pixelObj.getGreen() - pixelObj.getBlue())<15){	//check if green and blue are within 10 of each other
